...
cpu_quota=$(kubectl get resourcequota -n ns-8 -o jsonpath='{.items[0].status.hard.cpu}')
mem_quota=$(kubectl get resourcequota -n ns-8 -o jsonpath='{.items[0].status.hard.memory}')
