...
actual_total_deployments=$(kubectl get deployments --all-namespaces --no-headers 2>/dev/null | wc -l)
...
secret_total_deployments=$(kubectl get secret cluster-analysis -n default -o jsonpath='{.data.total-deployments}' 2>/dev/null | base64 -d)
...
if [ "$secret_total_deployments" != "$actual_total_deployments" ]; then
    exit 1
fi
